...
import com.qqb.travelclub.util.exception.ClubIntroLongerThanException;
import com.qqb.travelclub.util.exception.ClubNameLongerThanException;
...
public class TravelClub extends Entity {
	...
	private void checkNameValidation(String name) {
		...
		if (name.length() < TravelClub.MINIMUM_NAME_LENGTH) {
			throw new ClubNameLongerThanException("Name should be longer than " + TravelClub.MINIMUM_NAME_LENGTH);
		}
	}
	...
	private void checkIntroValidation(String intro) {
		...
		if (intro.length() < TravelClub.MINIMUM_INTRO_LENGTH) {
			throw new ClubIntroLongerThanException("Intro should be longer than " + TravelClub.MINIMUM_INTRO_LENGTH);
		}
	}
